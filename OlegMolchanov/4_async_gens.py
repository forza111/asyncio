...
def event_loop():
    while any([tasks, to_read, to_write]):
        ...
        while not tasks:
            ...
            print(f"{ready_to_read} - {to_read}, {ready_to_write} - {to_write}")


            for sock in ready_to_read:
                tasks.append(to_read.pop(sock))
                print(f"for sock in ready_to_read APPEND")
                print(sock)

            for sock in ready_to_write:
                tasks.append(to_write.pop(sock))
                print(f"for sock in ready_to_write APPEND")
        ...
        try:
            ...
            if reason == 'read':
                to_read[sock] = task
                print(f"dict << to_read >> appending {task} ")

            if reason == 'write':
                to_write[sock] = task
                print(f"dict << to_write >> appending {task} ")
